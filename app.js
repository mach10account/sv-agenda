...
// Finora il bot lavorava alla cieca: la titolare non vedeva cosa scriveva
// alle sue clienti. Qui ci sono le chat, e in cima una conversazione di
// prova che parla solo dentro il gestionale — serve a farsi un'idea del tono
// e delle disponibilita' proposte senza passare da WhatsApp.
...
async function apriConversazioni() {
  const { box, chiudi: chiudiPannello } = pannello();
  box.classList.add("scheda-larga", "scheda-chat");
  box.innerHTML = `<div class="scheda-testa"><h2>Conversazioni</h2></div>
                   <p class="quando">Caricamento…</p>`;
  ...
  const chiudi = () => { clearInterval(battito); chiudiPannello(); };

  await ricarica();
  // Se arriva un messaggio mentre il pannello e' aperto deve comparire da solo:
  ...
  async function ricarica(silenzioso = false) {
    const { data, error } = await sb.rpc("crm_wa_conversazioni", { p_centro: centro.id });
    if (error || !data?.ok) {
      if (silenzioso) return;
      box.innerHTML = `<div class="scheda-testa"><h2>Conversazioni</h2></div>
        ${erroreBox(error?.message || data?.errore || "Non disponibile")}
        <button class="btn btn-wide chiudi">Chiudi</button>`;
      return box.querySelector(".chiudi").onclick = chiudi;
    }
  ...
  function disegna(messaggio = "") {
    ...
    // Il pannello si ridisegna anche a ogni battito: quello che stava
    ...
    box.innerHTML = `
      <div class="scheda-testa">
        <h2>Conversazioni</h2>
        <div class="scheda-azioni"><button class="btn-x chiudi" title="Chiudi">✕</button></div>
      </div>
      ${messaggio ? erroreBox(messaggio) : ""}
    ...
    if (azzera) azzera.onclick = async () => {
      ...
      const { data, error } = await sb.rpc("crm_wa_prova_azzera", { p_centro: centro.id });
      if (error || !data?.ok) return disegna(error?.message || data?.errore || "Non riuscito");
  ...
  async function invia() {
    ...
    try {
      ...
      const res = await fetch(`${SUPABASE_URL}/functions/v1/wa-inbound`, {
        ...
        body: JSON.stringify({ centro_id: centro.id, testo }),
      });
      const esito = await res.json().catch(() => ({}));
      inviando = false;
      if (!esito?.ok) {
...
async function boot() {
  ...
  // Le chat delle clienti e le integrazioni le vede chi possiede il centro,
  // non chi ci lavora: sono conversazioni private, non materiale di reception.
  if (centro.ruolo === "titolare") {
    document.getElementById("integrazioni").hidden = false;
    document.getElementById("conversazioni").hidden = false;
  }
...
document.getElementById("conversazioni").addEventListener("click", apriConversazioni);
