...
export function mostraConversazioni() {
  ...
  async function ricarica(silenzioso = false) {
    ...
    //
    // E dev'essere una chat VERA, non la prova, anche se la prova sta in cima:
    // chi entra qui lavora con le clienti, e la prova è un'altra cosa — non ha
    // il bottone Template e i suoi messaggi non sono di nessuno. Aprirla per
    // prima faceva atterrare chiunque (compreso chi la sezione la vede per la
    // prima volta seguendo istruzioni scritte) nel posto dove metà dei comandi
    // non c'è. La prova resta a portata di clic, in testa all'elenco.
    if (primoGiro) {
      const vera = lista.find((c) => !c.prova);
      apertaId = (vera ?? lista[0])?.id ?? null;
      primoGiro = false;
    }
